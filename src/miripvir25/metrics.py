...
def assess_taxonomic_classification_metrics(reference: List[List[int]] , results: List[int], method: str):
    ...
    """
    results = list(set(results)) # De-replication
    reference_flat = list(set([item2 for item1 in reference for item2 in item1]))
    true_positives = 0
    false_positives = 0
    ...
    for i, x in enumerate(reference):
        if any([item in results for item in x]):
            tps.append(i)
        else:
            fns.append(i)
    fps = []
    for i, x in enumerate(results):
        if x not in reference_flat:
            fps.append(i)
